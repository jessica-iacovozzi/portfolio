...
export default function Button({ link, text, target }) {
  return (
    <a href={link} target={target ? "_blank" : ""} rel={target ? "noreferrer" : ""} className="w-fit text-lg relative inline-flex border-2 mt-10 border-white hover:border-gray-950 justify-start px-6 py-3 overflow-hidden font-medium transition-all bg-gray-950 rounded-lg hover:bg-gray-950 group">
